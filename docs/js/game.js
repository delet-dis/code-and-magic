...
window.Game = (function() {
  ...
  /**
   * Обновление движения файрбола.
   * Файрбол выпускается в определенном направлении
  ...
  LevelsInitialize[Level.INTRO] = function(state) {
    state.objects.push(
        ...
        // уровне равна 2px за кадр.
        {
          ...
          height: window.GameConstants.Wizard
              .getHeight(window.GameConstants.Wizard.width),
  ...
  Game.prototype = {
        ...
        case Verdict.WIN:
          if (window.renderStatistics) {
            const statistics = this
                ._generateStatistics(new Date() - this.state.startTime);
     ...
     * появиться,
     * выполняет проверку поведения всех объектов и удаляет те, которые
     * должны исчезнуть.
    ...
    updateObjects: function(delta) {
      ...
      if (this.state.keysPressed.SHIFT) {
        this.state.objects.push({
          ...
          speed: window.GameConstants
              .Fireball.speed(!!(me.direction & Direction.LEFT)),
          sprite: SpriteMap[ObjectType.FIREBALL],
          type: ObjectType.FIREBALL,
          width: window.GameConstants.Fireball.size,
          x: me.direction & Direction.RIGHT ? me.x +
               me.width : me.x - window.GameConstants.Fireball.size,
          y: me.y + me.height / 2,
        });
    ...
    checkStatus: function() {
      ...
      if (!this.commonRules) {
        ...
        this.commonRules = [
           ...
           *  игрок продолжает играть в нее два часа подряд.
           * @param {Object} state
           * @return {Verdict}
           */
          function(state) {
    ...
    render: function() {
      ...
      this.state.objects.forEach(function(object) {
        if (object.sprite) {
          ...
          const sprite = SpriteMap[object.type +
               (reversed ? REVERSED : '')] || SpriteMap[object.type];
